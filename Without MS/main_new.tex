...
\[(TC_{\rho}\sV^0(B, \sP'))^{1,0} = \pi(\SS(\sP').\]
We then conclude 
that $\varphi^*(\Char(B))\subset \sV^0(A, \sP)$ from observing that
\[H^0(B, \sP'\otimes \bbC_{\rho'}) \overset{\oplus}{\into}
H^0(A, \sP\otimes \bbC_{\rho'}).\]
To see the converse not that 
for any $\rho \in \sV^0(A, \sP)$ we must 
have $\bbC_{\rho}|_{\operatorname{Fibre}(\varphi)} = \bbC$.
Indeed, $H^0(A, \sP\otimes \bbC_{\rho}) = H^0(B, \sP'\otimes \varphi_*\bbC_{\rho})$.
