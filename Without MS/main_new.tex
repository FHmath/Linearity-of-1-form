...
\cite{BWY}. However,
We do know at the moment whether the equality holds in $\sR(X) \subseteq V(X)$. \sorry{isn't this question in fact equivalent to the question
raised by Schreieder \cite{Sch} that there are no universally
non-resonant forms.}
\end{remark}
...
We have the following (see e.g.\ Theorem 17.9)
\begin{theorem}
...
if $\sL = \Psi(\rho)$ then
\[\sR^k(X, \bbC_{\rho}) =\bigcap_{p+q = k} \{\omega| H^{p,q}(X, \sL,\omega) \neq 0\}.\]
\end{remark}
...
\section{Linearity of forms with codimension 1 zeros}
...
\begin{theorem} \label{log-linear-v1}
