...
We call a holomorphic 1-form $\omega\in V(X)$ (non)\emph{resonant} if the complex 
...
Such a 1-form $\omega\in H^0(X, \Omega_X^1)$ is called universally nonresonant if the complex 
\[\ldots\to H^{i-1}(X',\C)\overset{\wedge\tau^*\omega}{\longrightarrow}H^{i}(X',\C)\overset{\wedge\tau^*\omega}{\longrightarrow}H^{i+1}(X',\C)\to\ldots\]
 is exact for any \'etale over $\tau: X'\to X$. 
...
\begin{enumerate}
\setcounter{enumi}{-1}
	\item  For any 1-form $\omega$ for which the resonance sequence $(H^{\bullet}(X,\bbC), \wedge\omega)$ is not exact, it must be the case that $\omega\in V(X)$. This is a consequence of the generic vanishing package. Since we do not use this theory much, we only 
	outline an argument here with relevant references. Assuming the contrary,
	otherwise the Koszul complex (see e.g.\ \cite[B.2 Complexes]{Laz04a} associated to the global section $\omega$ 
...
\item In general by \cite[Proposition 3.4]{GL87}, if a holomorphic $\omega$ has zero scheme of codimension greater than or equal to $k$, then the complex $$\ldots\to H^{i}(X,\C)\overset{\wedge\omega}{\rightarrow}H^{i}(X,\C)\overset{\wedge\omega}{\rightarrow}H^{i}(X,\C)\to\ldots$$ is exact whenever $i<k.$ 

 
\item As mentioned in \cite[1.6]{SS19}, nonresonant holomorphic 1-form exists for bielliptic surfaces with multiple fibers. 
\end{enumerate}
\end{remark}
