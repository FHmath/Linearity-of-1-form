...
Results in this section are taken from \cite{MiSh18}. For a morphism $f: X\to A$ as in Theorem \ref{main1},  we consider the following diagram 
%$$\xymatrix{
%T^*X 
...
\begin{remark}
\begin{enumerate}
	\item By [\emph{loc.\ cit.}, Lemma 2.8], in the above definition an $i-1$-dimensional subspace $W$ of $T_yY$ is transversal to $f_{\dagger}(0_X)$ means 
\[df_x(T_xX)+W=T_yY,\]
 for any closed point $x\in f^{-1}(y)$. 
%Thus no $i-1$-dimensional subspace of $T_yY$ is transversal to $f$ means the rank of $f$ at some point in $f^{-1}(y)$ is less than 
This is equivalent to the condition that $\dim df_x(T_xX) \leq n-i$, where $n=\dim Y$.

\item By definition, $\Delta^1(f)$ is the usual discriminant, i.e., the locus in $Y$ such that $f$ is not smooth. Also by \cite[Proposition 10.6]{Har77}, we have the filtration  $$Y=\Delta^0(f)\supset \Delta^1(f)\supset\Delta^2(f)\supset\Delta^3(f)\supset\ldots.$$

\item $\Delta^i(f)$ are closed \sorry{reference?} subvarieties of $Y$, which measure how singular $f$ can be.
\end{enumerate}
\end{remark}
...
\begin{proof}
 Indeed,
by a result of Ueno \cite[]{Ueno} we know that
