...
\begin{remark}
(1) An $i-1$-dimensional subspace $W\subset T_yY$ for a point $y\in Y$ is said to be transversal to $f$ means $$df_x(T_xX)+W=T_yY,$$ for any closed point $x\in f^{-1}(y)$. Thus the condition in the Definition \ref{def:higherdiscriminants} means that for any such $W$, 
there exists $x\in f^{-1}(y)$ such that $\dim df_x(T_xX) \leq n-i$.
%Thus no $i-1$-dimensional subspace of $T_yY$ is transversal to $f$ means the rank of $f$ at some point in $f^{-1}(y)$ is less than $n-i$, where $n=\dim Y$.
...
Since the codimension $i$ components $D^i(f)$ of $\Delta^i(f)$ will be used for many times, we fix the following notation.
...
(3) \sorry{TBA necessary and sufficient condition for Kotschick's conjecture: The subabelian variety spanned by the subabelian 
varieties that fiber $D^i$ for some $i$ is a proper subabelian variety.}
\end{remark}
