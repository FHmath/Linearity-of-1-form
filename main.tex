...
$X$ is of general type. By a result of Ueno \cite[Theorem 10.9]{Uen75} this is equivalent to $X$ being not fibered by any subabelian varieties of $A$. 
When $X$ is smooth 
...
In this section, we recall a very powerful tool by Migliorini  and Shende \cite{MiSh18}, which will be used in the proof of Theorem \ref{main1}.
...
$$\Delta^i(f):=\{ y\in Y\ |\ \textnormal{no}\ (i-1)\textnormal{-dimensional subspace of}\  T_yY \ \textnormal{is transversal to} \ f_{\dagger}(0_X) \},$$ for each nonnegative integer $i$.
\end{definition}

\begin{remark}
(1) By \cite[Lemma 2.8]{MiSh18}, in the above definition an $i-1$-dimensional subspace $W$ of $T_yY$ is transversal to $f_{\dagger}(0_X)$ means that \[df_x(T_xX)+W=T_yY,\] for any closed point $x\in f^{-1}(y)$. 
...
As a corollary we obtain a strengthening of \cite[Theorem ?]{HK05}.
\begin{corollary}\label{cor:hk}
Let $f\colon X\to A$ be as above.
Suppose that for some $i$, $D^i$ admits a component of general
type 
then every global holomorphic 1-form on $X$ coming from $A$
admits zero.
\end{corollary}

\begin{proof}
As we have noted before
by a result of Ueno \cite[Theorem 10.9]{Uen75} 
subvarieties of general type on an abelian variety
are precisely the ones that are not fibered by tori.
\end{proof}
