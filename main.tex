...
Not too much information is known for nonresonant 1-forms. The following theorem is due to S. Schreieder, which studies some properties of zeros of nonresonant 1-forms and the albanese maps of varieties admitting nonresonant 1-forms.
...
where for a vector bundle $E$ on $X^{\reg}$ we use the notation $\bbP(E) \coloneqq \Proj(\Sym^d(E^{\vee}))$.
...
\end{equation}
Denote by $S\coloneqq \varphi(\bbP(N))$ the image and by $m = \dim S$. If $m<g$, for all $s\in S$, a general
 fibre $Z_s \coloneqq \varphi^{-1}(s)$
must have dimension $g - m - 1$. Let $A_0$ denote the abelian variety spanned by $Z_s$, then $\dim A_0 > g- m - 1$. Indeed,
...
is cut out by $g - m -1$ many hyperplanes in $T_0A$. But $\dim T_0A_0 \gneq \dim T_0A$. Hence $\varphi$ is quasi-finite
...
By lemma \ref{countable}, $V_{\textnormal{neg}}$ is a union of at most countably many linear subspaces in $H^0(X, \Omega_X^1)$ of the following type $$V_E(X):=\{\omega\in H^0(X, \Omega_X^1)\ |\ \omega(p)=0\  \textnormal{for any point}\ p\in E\},$$ where $E$ is a negative divisor. Since $V^1(X)$ is an algebraic set and $T_h^1(X)$ is a linear subvariety, $V^1(X)$ is a linear subvariety.
 ...
 \begin{proposition}
Let $\omega$ be a holomorphic nonresonant 1-form. By  There are only finitely many integral divisors $D$ on $X$, such that there is a nonresonant 1-form $\omega\in H^0(X, \Omega_X^1)$ with $\omega(D)=0$.
 \end{proposition}
...
\bibitem[Deb07]{Deb} O. Debarre,  \textit{On the geometry of abelian varieties} \url{https://www.math.ens.fr/~debarre/Taiwan.pdf}, (2007).
