...
$$\sV^i(X)=\lbrace \rho\in \textnormal{M}_B(X) \mid  H^{i}(X, L_{\rho})\neq 0 \rbrace,$$ 
which by structure theorem are finite union of torsion translated subtori of $\textnormal{M}_B(X)$.

To study $V^1(X)$ we introduce a homotopy invariant of $X$, denoted by $T^1(X)$. 



The following definition of $T^1(X)$ is based on Arapura's work One can give an equivalent definition of $R^1(X)$ based on Arapura's work. Roughly speaking, Arapura showed that every positive dimension component of $\sV^1(X)$ can be induced by a so called orbifold map. I will just give some references here. You can just read \cite[section 9,10]{Su}. Some relevant references are \cite{A,AC,ACM}.

To define $R^1(X)$, one indeed only needs a smaller class of orbifold maps (by Stein factorization theorem). 
 An algebraic map $f:X\to \Sigma$ from $X$ to a smooth projective curve $\Sigma$ is called a  {\it strict orbifold map} if $f $ is surjective, has connected fibers and one of the following condition holds:
\begin{itemize}
\item $g(\Sigma)\geq 2$
\item $g(\Sigma)=1$ and $f$ has at least one multiple fiber.
\end{itemize}
A strict orbifold map always induces an injective map: $f^*: H^0(\Sigma, \Omega_\Sigma^1) \to H^0(X,\Omega^1_X)$. 
There only at most finitely many equivalent strict orbifold maps. Define $$R^1(X):= \bigcup_f \im f^*,$$
where the union runs over all possible strict orbifold maps for $X$.
