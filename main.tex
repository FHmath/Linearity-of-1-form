...
\subsection{Higher discriminants and singularities of morphisms by Migliorini and Shende}

In this subsection, we recall a very powerful tool by Migliorini  and Shende \cite{MiSh18}, which will be used in the proof of Theorem \ref{main1}.
...
For any morphism $f: X\to Y$,  we consider the following diagram 
$$\xymatrix{
T^*X 
& X\times_Y T^*Y \ar[l]_-{df} \ar[r]^-{\tilde{f}} &T^*Y
},$$ where $T^*X$ and $T^*Y$ are holomorphic cotangent bundle, $df$ is the usual differential, and $\tilde{f}$ is the natural projection. Now we can formulate the following theorem in \cite[Theorem C]{MiSh18}.
...
\subsection{Case for general abelian variety}
...
In fact, since $Y_1$ is not fibred by tori, $Y_1$ is of general type 
\end{proof}
